...
- **실시간 사용량 추적** — Anthropic API 또는 로컬 JSONL 로그 기반 5시간/주간 rate limit 사용률
...
**Swift 6.2** 및 **macOS 26** (Tahoe) SDK 필요.
...
| 소스 | 동작 방식 |
|------|----------|
| **Anthropic API** | `~/.claude/credentials.json`의 OAuth 토큰 — 실시간 5시간/주간 버킷, Enterprise 할당량 |
| **로컬 JSONL** | `~/.claude/projects/**/usage.jsonl` 폴백 — 로그 기반 토큰 수 추정 |
...
- **Enterprise 할당량 카드** — 개인/티어/조직 지출 한도 (Enterprise 플랜 전용)
- **비용 카드** — 5시간 및 일일 비용 추정
- **토큰 분석** — 타입별 가중치 적용 토큰 사용량
- **Rate limit 필** — 5h / 7d / Sonnet 버킷 사용률

### 플로팅 필

오버레이 필은 평소 잔여 퍼센트를 간결하게 표시하며, 호버 시 다음 정보로 확장됩니다:
