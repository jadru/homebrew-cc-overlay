...
| Recommendation trust | Local helpful votes / all recommendation votes, triangulated with interviews | 70% and 7 of 10 | Make recommendation the core wedge |
| Guided-run success | Completed outcomes / guided runs with an outcome | 80% | Keep Run / Switch as the primary action |
| Limit-hit rate | Limit-hit outcomes / completed or limit-hit outcomes | Below 10% | Trust the task-fit threshold |
...
- Task-fit samples, guided-run outcomes, and helpful/not-helpful recommendation votes remain local; safe diagnostics expose aggregate counts only.
- GitHub release downloads, Homebrew installs, issues, and opt-in interviews are the distribution proxies.

## Weekly Review

Every Monday record visitor, CTA, release, activation-sample, guided-run outcome, issue, and interview counts. Keep raw counts beside percentages; cohorts below 30 users are directional only.
