...
- **로컬 Codex 토큰 폴백** — Codex app-server 빌드가 토큰 활동량을 반환하지 못해도 최근 로컬 Codex 롤아웃의 토큰 카운터로 컴패니언 진행도가 계속 누적됩니다.
- **과거 사용량 재보상 방지** — 첫 로컬 누적값은 앱 시작 기준값으로만 사용하고, CC-Overlay가 실행 중인 동안의 새 작업만 토큰으로 반영합니다.
- **호환되는 app-server 요청** — 파라미터 없는 Codex app-server 요청을 스키마 형식으로 보내고, 초기화 응답을 받은 뒤 계정 요청을 보냅니다.
- **제한된 저널 스캔** — 최근 활성 저널만 증분으로 읽어 전체 Codex 아카이브를 반복해서 훑지 않습니다.
